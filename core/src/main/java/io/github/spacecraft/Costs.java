...
public class Costs {
    ...
    static int getClickLevelCost(int currentClickLevel) {
        int cost = (int) pow(2,Math.floor(currentClickLevel/5f));
        clickLevelCost = cost;
        return cost;
    }

    public static int getIdleLevelCost(int currentIdleLevel) {
        int cost = (int) pow(1.5,Math.floor(currentIdleLevel/5f));
        idleLevelCost = cost;
        return cost;
    }

    public static int getNavigatorLevelCost(int currentNavigatorLevel) {
        int cost = (int) pow(3,Math.floor(currentNavigatorLevel/5f));
        navigatorLevelCost = cost;
        return cost;
    }

    public static int getHarvestTimeLevelCost(int currentHarvestTimeLevel) {
        int cost = (int) pow(1.7,Math.floor(currentHarvestTimeLevel/5f));
